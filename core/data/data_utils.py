...
def tokenize_question(question, use_glove):
    token_to_ix = {
        'PAD': 0,
        'UNK': 1,
    }
    ...
    if use_glove:
        spacy_tool = en_vectors_web_lg.load()
        emb.append(spacy_tool('PAD').vector)
        emb.append(spacy_tool('UNK').vector)
    ...
    for word in words:
        if word not in token_to_ix:
            token_to_ix[word] = len(token_to_ix)
            if use_glove:
                emb.append(spacy_tool(word).vector)

    emb = np.array(emb)

    return token_to_ix, emb
